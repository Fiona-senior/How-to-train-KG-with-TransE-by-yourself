...
class TransE:
    ...
    def train(self, epochs):
        ...
        for epoch in range(epochs):
            ...
            # 保存每一个batch的临时结果
            if epoch % 10 == 0:
                with codecs.open("../vc_res/entity_temp", "w") as f_e:
                    for e in self.entity.keys():
                        f_e.write(str(e) + "\t")
                        f_e.write(str(list(self.entity[e]))) #  self.entity = entity_dict  # 初始化后的实体   {实体编号：L2范数归一化后的向量}
                        f_e.write("\n")
                with codecs.open("../vc_res/relation_temp", "w") as f_r:
                    for r in self.relation.keys():
                        f_r.write(str(r) + "\t")
                        f_r.write(str(list(self.relation[r])))
                        f_r.write("\n")
                with codecs.open("../vc_res/result_temp", "a") as f_s:
                    f_s.write("epoch: %d\tloss: %s\n" % (epoch, self.loss))

        print("写入文件...")
        with codecs.open("../vc_res/entity_50dim_batch400", "w") as f1:
            for e in self.entity.keys():
                f1.write(str(e) + "\t")
                f1.write(str(list(self.entity[e])))
                f1.write("\n")

        with codecs.open("../vc_res/relation_50dim_batch400", "w") as f2:
            for r in self.relation.keys():
                f2.write(str(r) + "\t")
    ...
    def Corrupt(self, triple):  # 随机替换头实体和尾实体制作负样本
        ...
        if seed > 0.5:
            ...
            while (rand_head == head):
                rand_head = random.choice(list(self.entity))  # 从实体集合随机选择一个实体作为头实体替换
            ...
            while (rand_tail == tail):
                rand_tail = random.choice(list(self.entity))  # 从实体集合随机选择一个实体作为尾实体替换
            corrupted_triple[2] = rand_tail
        return corrupted_triple

    def update_embeddings(self, Tbatch):
        # 这里深拷贝了整个字典,会造成训练速度较慢，但是是易于理解的，增加训练速度的版本是transE_speed
        copy_entity = copy.deepcopy(self.entity)   # 31062
        # print("copy_entity:",copy_entity.keys())
        copy_relation = copy.deepcopy(self.relation)   # 16650

        for triple, corrupted_triple in Tbatch:
            # 取copy里的vector累积更新
            h_correct_update = copy_entity[triple[0]]  # 取正确三元组的头实体的（初始化）向量
            # print("triple[2]:",triple[2])
            t_correct_update = copy_entity[triple[2]]  # 取正确三元组的尾实体的（初始化）向量
            # print("triple[1]:",triple[1])
            relation_update = copy_relation[triple[1]]  # 取正确三元组的关系的（初始化）向量

            h_corrupt_update = copy_entity[corrupted_triple[0]]  # 取错误三元组的头实体的(初始化)向量
            # print("corrupted_triple[2]:",corrupted_triple[2])
            t_corrupt_update = copy_entity[corrupted_triple[2]]  # 取错误三元组的尾实体的(初始化)向量

            # 取原始的vector计算梯度
            h_correct = self.entity[triple[0]]  # 正确三元组的头实体向量
            t_correct = self.entity[triple[2]]  # 正确三元组的尾实体向量
            relation = self.relation[triple[1]]  # 正确三元组的关系向量

            h_corrupt = self.entity[corrupted_triple[0]]  # 错误三元组的头实体向量
            t_corrupt = self.entity[corrupted_triple[2]]  # 错误三元组的尾实体向量
